...
function proxyStatus(d){if(d.is_proxy===true||d.is_proxy===1||String(d.is_proxy).toLowerCase()==="true")return "Proxy/VPN";const u=String(d.usage_type||"").toUpperCase();if(["DCH","DATACENTER","DATA CENTER","HOSTING","HOSTING/DATACENTER"].some(x=>u.includes(x)))return "Hosting/Datacenter";if(["ISP","ISP/MOBILE","MOB","MOBILE"].some(x=>u.includes(x)))return "Residential";return d.usage_type||"Not detected"}
...
try{
  ...
  await new Promise((resolve,reject)=>{const {exec}=require("node:child_process");exec("node scripts/build-index.mjs && git config user.name PingMap-Bot && git config user.email 41898282+github-actions[bot]@users.noreply.github.com && git add data && git commit -m \"Add IP ${ip}\" && git push",(err,stdout,stderr)=>err?reject(new Error(stderr||err.message)):resolve(stdout)});});
